...
if (envResult.error && app.isPackaged) {
    console.log('Failed to load .env from resources, trying alternative location...');
...
function createWindow() {
    ...
    mainWindow.on('close', async (event) => {
        if (serverProcess) {
            ...
            if (!result.success) {
                console.error('Failed to stop server during window close:', result.message);
            }
...
async function stopAppiumProcess() {
    ...
    try {
        ...
        if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            const message = results.length > 0 ? results.join(', ') : 'Appium and emulator were not running';
            mainWindow.webContents.send('server-log', {
                type: 'info',
                message: message
            });
