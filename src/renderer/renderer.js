class TestRunnerUI {
  ...
  bindElements() {
    ...
    this.localRunnerStatusText = document.getElementById(
      'local-runner-status-text'
    );
    this.localRunnerStatusDot = document.getElementById(
      'local-runner-status-dot'
    );
  ...
  async startServer() {
    ...
    try {
      ...
      this.addLogEntry(
        'local-runner',
        'error',
        `Failed to start local runner: ${error.message}`
      );
    } finally {
      this.updateUI();
    }
  ...
  async stopServer() {
    ...
    try {
      ...
      this.addLogEntry(
        'local-runner',
        'error',
        `Failed to stop local runner: ${error.message}`
      );
    } finally {
      this.updateUI();
    }
  ...
  async startAppium() {
    ...
    this.startAppiumBtn.innerHTML =
      '<span class="btn-icon">⏳</span>Starting...';
  ...
  async stopAppium() {
    ...
    this.stopAppiumBtn.innerHTML =
      '<span class="btn-icon">⏳</span>Stopping...';
    ...
    try {
      ...
      this.addLogEntry(
        'local-runner',
        'error',
        `Failed to stop Appium: ${error.message}`
      );
    } finally {
      this.updateAppiumUI();
    }
  ...
  clearLogs() {
    ...
    if (this.activeLogTab === 'local-runner') {
      ...
      this.addLogEntry(
        this.activeLogTab,
        'info',
        `${this.activeLogTab.toUpperCase()} logs cleared`
      );
    }
  }
